		...
		<NcAppContent>
			<div class="gptzero-app">
				<div class="gptzero-app-heading">
					...
					<p class="description">
						{{ t('integration_gptzero', 'This app allows you to check if some text is likely to be generated by an AI model.') }}
					</p>
					<p class="pre-caution">
						<InformationOutlineIcon :size="20" class="icon" style="margin-right: 5px;" />
						{{ t('integration_gptzero', 'All the text and files you submit will be sent to the GPTZero service. Please do not use any sensitive data.') }}
					</p>
				</div>
				<textarea v-if="selectedFiles.length === 0"
					...
					cols="80"
					rows="5"
					:placeholder="t('integration_gptzero', 'Type or paste the text for which you want to detect AI implication (minimum 250 characters)')" />
